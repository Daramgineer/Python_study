...
def merge_image(): #세로로 나열하도록 통합
    try:
        ...
        if img_width > -1:
            images_sizes = [(int(img_width), int(img_width * x.size[1] / x.size[0]))]
        else:
            img_sizes = [(x.size[0], x.size[1]) for x in images] #원본 적용

        #100*60 이미지 -> width 80, height?
        #원본의 비율을 맞춰서 조절.

        widths, heights = zip(*(img_sizes)) #*붙이면 각 첫번쨰, 두번쨰 성분들 묶어서 전달

        #최대넓이 전체높이 구하기
        max_width, total_height = max(widths), sum(heights) #가로 최대, 세로 총합

        #스케치북 준비
        if img_space > 0: #이미지 간격옵션 적용
        ...
        for idx, img in enumerate(images):
            #width가 원본이 아닌경우 크기 조정
            if img_width > -1:
                img = img.resize(image_sizes[idx]) #리사이징

            result_img.paste(img, (0, y_offset))
            y_offset += (img.size[1] + img_space) #간격 더해주기
